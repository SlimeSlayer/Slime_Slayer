...
bool Tutorial::Enable()
{
	if (!base_enabled)
	{
		...
		rect = App->entities_manager->TokenStrToRect(current_enable_node.attribute("rect").as_string());
		end_trigger = App->physics->CreateRectangle(rect.x, rect.y, rect.w, rect.h, COLLISION_TYPE::MAP_COLLISION, BODY_TYPE::MAP_BODY);
